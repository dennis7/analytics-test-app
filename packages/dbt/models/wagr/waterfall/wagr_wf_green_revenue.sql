{{ config(alias='wf_green_revenue') }}

WITH weights AS (
    SELECT * FROM {{ ref('wagr_harm_portfolio_weights') }}
),

entity_green_revenue AS (
    SELECT * FROM {{ ref('stg_entity_green_revenue') }}
),

hierarchy AS (
    SELECT * FROM {{ ref('stg_entity_hierarchy') }}
),

sector_green_revenue AS (
    SELECT * FROM {{ ref('stg_sector_green_revenue') }}
)
