{% macro shared_harm_valued_positions(metric) %}

WITH public_securities AS (
    SELECT * FROM {{ ref(metric ~ '_prep_public_securities') }}
),

private_securities AS (
    SELECT * FROM {{ ref(metric ~ '_prep_private_securities') }}
),

market_data AS (
    SELECT * FROM {{ ref('stg_public_market_data') }}
),

private_valuations AS (
    SELECT * FROM {{ ref('stg_private_valuations') }}
),
